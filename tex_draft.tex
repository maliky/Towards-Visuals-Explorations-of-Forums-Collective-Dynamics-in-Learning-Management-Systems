...
In thread 1 (Figure~\ref{fig:discussion}), some relation between $\T$ and $\U$ would occur for the same reason than that of $A \dashrightarrow B$.  Incidentally, in the \nth{2} thread, we would have $\T \dashrightarrow \V$ based on time proximity, but also $\T \dashrightarrow \U$ based on the shared author $A$.
...
% pre correction grammarly
% (shell-command "detex tex_draft.tex " nil nil)
