...
% We define the causal interaction's strength $I$ between two messages as the measure of interaction taking in account the intention of the exchange.  It is a function of $s$ and $r$, where $s$ is the implicite relationship's strengh, independent of the intention.  It is based on topic, time and actor closeness. And $r$ is the required message correlation, defined by as the observer's evaluation of how much implicite relationship is required to explicit it.  For all $s \in ]0;1[ $, $I$ should satisfy:
...
\begin{figure*}[t]
  ...
  \small{
    \caption{\label{fig:evolution}
      At the top (a) is half of compound yearly actor-actor network built.  The three bottom images (a), (b) and (c) are closeup around actor 578 during the \nth{2}, \nth{3} and \nth{4} quarters of the year.  We can see how the network changes around him.  }}
\end{figure*}
